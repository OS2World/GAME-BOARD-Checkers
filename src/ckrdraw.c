...
HPS CkdCreatePS (HWND hwnd)
     ...
     hdcMemory2 = DevOpenDC (hab, OD_MEMORY, "*", 0L, NULL, NULLHANDLE) ;
     ...
     PrfQueryProfileData (HINI_USERPROFILE, szApplication, szClrBackground,
                          &clrBackground, &sDataSize) ;

     sDataSize = sizeof (LONG) ;
     PrfQueryProfileData (HINI_USERPROFILE, szApplication, szClrBlackSquare,
                          &clrBlackSquare, &sDataSize) ;

     sDataSize = sizeof (LONG) ;
     PrfQueryProfileData (HINI_USERPROFILE, szApplication, szClrWhiteSquare,
                          &clrWhiteSquare, &sDataSize) ;

     sDataSize = sizeof (LONG) ;
     PrfQueryProfileData (HINI_USERPROFILE, szApplication, szClrBlackPiece,
                          &clrBlackPiece, &sDataSize) ;

     sDataSize = sizeof (LONG) ;
     PrfQueryProfileData (HINI_USERPROFILE, szApplication, szClrWhitePiece,
...
BOOL CkdDestroyPS (HPS hps)
     ...
     PrfWriteProfileData (HINI_USERPROFILE, szApplication, szClrBackground,
                          &clrBackground, sizeof (LONG)) ;

     PrfWriteProfileData (HINI_USERPROFILE, szApplication, szClrBlackSquare,
                          &clrBlackSquare, sizeof (LONG)) ;

     PrfWriteProfileData (HINI_USERPROFILE, szApplication, szClrWhiteSquare,
                          &clrWhiteSquare, sizeof (LONG)) ;

     PrfWriteProfileData (HINI_USERPROFILE, szApplication, szClrBlackPiece,
                          &clrBlackPiece, sizeof (LONG)) ;

     PrfWriteProfileData (HINI_USERPROFILE, szApplication, szClrWhitePiece,
     ...
     for (sKing = 0 ; sKing < 2 ; sKing++)
          ...
          for (sColor = BLACK ; sColor <= WHITE ; sColor++)
               ...
               ahbmPiece[sColor][sKing] =
                         GpiCreateBitmap (hps, (PBITMAPINFOHEADER2)&bmp, 0L, 0L, NULL) ;
          ...
          ahbmMask[sKing] = GpiCreateBitmap (hps, (PBITMAPINFOHEADER2)&bmp, 0L, 0L, NULL) ;
          GpiSetBitmap (hpsMemory, ahbmMask[sKing]) ;
          CkdRenderPieceBitmap (hpsMemory, CLR_TRUE, CLR_FALSE, CLR_FALSE, sKing) ;

                    // Create ahbmSave bitmaps

          ahbmSave[sKing] = GpiCreateBitmap (hps, (PBITMAPINFOHEADER2)&bmp, 0L, 0L, NULL) ;
          ...
          ahbmMove[sKing] = GpiCreateBitmap (hps, (PBITMAPINFOHEADER2)&bmp, 0L, 0L, NULL) ;
          }
...
VOID CkdDragMove (HPS hps, POINTL *pptlFrom, POINTL *pptlTo,
     ...
     GpiSetBitmap (hpsMemory2, NULLHANDLE) ;
     }
